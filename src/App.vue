      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <nav class="flex flex-wrap gap-2 p-1 bg-white/50 dark:bg-gray-800/50 rounded-2xl border border-[#141414]/5 dark:border-gray-700 w-fit">
            <button v-for="tab in [
              { id: 'items', label: 'Discovery', icon: 'lucide:layout-grid', count: stats.totalItems },
              { id: 'channels', label: 'All Talent', icon: 'lucide:users', count: stats.totalChannels },
              { id: 'collected_channels', label: 'Collected Talent', icon: 'lucide:heart', count: stats.collectedChannels },
          ...
          <div v-if="activeTab !== 'search'" class="flex items-center gap-3">
            ...
            <select v-if="activeTab === 'items' || activeTab === 'collected_playlists'" v-model="itemSort" @change="fetchData()" class="bg-white dark:bg-gray-800 border border-[#141414]/10 dark:border-gray-600 rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-[#5A5A40] dark:focus:ring-amber-500 outline-none">
          ...
          <div v-for="item in (activeTab === 'search' ? searchResults.filter(r => r.type === 'video' || r.type === 'playlist') : items)" :key="item.id" class="group bg-white dark:bg-gray-800 rounded-3xl overflow-hidden border border-[#141414]/5 dark:border-gray-700 hover:border-[#141414]/20 dark:hover:border-gray-600 transition-all hover:shadow-2xl hover:shadow-black/5 dark:hover:shadow-black/20">
            <div class="aspect-video relative overflow-hidden">
              ...
              <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6 gap-2">
                ...
                <button @click.stop="toggleCollect(item, item.type || 'video')" class="w-12 h-12 bg-white/20 backdrop-blur-md text-white rounded-xl flex items-center justify-center hover:bg-white/40 transition-colors">
                  <iconify-icon icon="lucide:bookmark" width="20" :class="{ 'fill-white': item.is_collected }"></iconify-icon>
                </button>
              </div>
        ...
        <div v-if="activeTab === 'channels' || activeTab === 'collected_channels' || activeTab === 'search'" :class="['grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6', activeTab === 'search' ? 'mt-12 border-t border-[#141414]/10 dark:border-gray-700 pt-12' : '']">
          ...
          <div v-for="channel in (activeTab === 'search' ? searchResults.filter(r => r.type === 'channel') : channels)" :key="channel.id" class="group bg-white dark:bg-gray-800 p-6 rounded-3xl border border-[#141414]/5 dark:border-gray-700 flex flex-col items-center text-center hover:shadow-xl transition-all relative">
            <button @click.stop="toggleCollect(channel, 'channel')" class="absolute top-4 right-4 text-[#141414]/20 dark:text-gray-500 hover:text-[#5A5A40] dark:hover:text-amber-400 transition-colors">
              <iconify-icon icon="lucide:heart" width="18" :class="{ 'fill-[#5A5A40] text-[#5A5A40] dark:fill-amber-400 dark:text-amber-400': channel.is_collected }"></iconify-icon>
            </button>
